# -*- coding: utf-8 -*-
# @Author: YangZhou
# @Date:   2016-10-17 16:26:39
# @Last Modified by:   YangZhou
# @Last Modified time: 2017-06-18 23:14:19
...
""" % (boltz_2d_dir, nkfs, cell, pos, nks, aa)
        # pw2wannier90 make some operation to cell and finally
        # judge rcell == acell (<1e-6) and it's too strict
...
/"""
    ...
    def runscf(self):
        if not tl.exists('scf.in'):
            self.getscfin()
        pw = self.getx("pw")
        tl.passthru(pw + " <scf.in > scf.out")
...
""" % nqs
    ...
    def get_amass(self):
        ...
        amass = '\n'.join(
            ['amass(%d)=%f' % (i + 1, s) for i, s in enumerate(masses)])
        return amass
...
""", "pedos.in")
    ...
    def getx(self, s='pw', pa=True, pool=True):
        ...
        s = config.mpirun + str(subcores) + " " + \
            config.espresso + "bin/" + s + ".x "
        if pool:
...
50.0 0.0""", "plotband.in")
 ...
 /""", "q2r.in")
        tl.shell_exec(config.espresso + "bin/q2r.x < q2r.in >q2r.out")
