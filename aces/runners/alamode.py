...
# @Last Modified time: 2017-06-18 22:21:56

from aces.tools import  shell_exec,\
    exists, write, passthru, read
...
class runner(Runner):
    ...
    def run_anphonin(self):
        passthru(config.mpirun + str(self.m.cores) +
                 config.anphon + " band.in > band.out")
        passthru(config.mpirun + str(self.m.cores) +
                 config.anphon + " dos.in > dos.out")
        passthru(config.mpirun + str(self.m.cores) +
                 config.anphon + " tc.in > tc.out")
    ...
    def getdispforce(self, files):
        ...
        for dir0 in files:
            forcearr = read_forces(
                'dirs/dir_%s/vasprun.xml' %
                dir0) / 25.7110  # in Rd/bohr
            force += matrixFormat(forcearr)
            disparr = (
                io.read(
                    'dirs/dir_%s/POSCAR' %
                    dir0).positions - orig) * 1.889726  # in bohr
        ...
        pos = '  \n'.join(
            [
                ...
                (m.elements.index(
                    ...
                    1) +
                m.toString(
                    atoms.get_scaled_positions()[i]) for i,
                a in enumerate(atoms)])
        position = """&position
	%s
/
""" % pos
