...
def mapatoms(newatoms, oldatoms):
    ...
    # d2s[j,ii,jj] means the distance of atom ii and atom jj with ii in
    # unitcell but jj in translated[ja,jb,jc] unitcell
    d2s = np.empty((27, len(atoms), len(oldatoms)))
    ...
    # d2min[ii,jj] means the nearest distance of atom ii and atom jj consider
    ...
    print("mapatoms-distances:",
          np.array([d2min[i, map0[i]] for i in range(len(newatoms))]))
    return map0
