# -*- coding: utf-8 -*-
# @Author: YangZhou
# @Date:   2017-06-19 13:09:33
# @Last Modified by:   YangZhou
# @Last Modified time: 2017-06-19 13:09:39
import numpy as np
import pandas as pd
...
def plot_dos():
    df = pd.read_csv("VDOS.txt", sep=r"[ \t]", engine="python")
    freq, dosx, dosy, dosz = df['Freq_THz'], df[
        'vdos_x'], df['vdos_y'], df['vdos_z']
    series(xlabel='Frequency(THz)',
           ylabel='Phonon Density of States',
           datas=[(freq, dosx, "dos_x"),
                  (freq, dosy, "dos_y"),
                  (freq, dosz, "dos_z")], linewidth=0.3, filename='VDOS.png')
...
def plot_vacf():
    ...
    series(xlabel='Correlation Time (ps)',
           ...
           datas=[(time, vx, "vcf_x"),
                  (time, vy, "vcf_y"),
                  (time, vz, "vcf_z")], linewidth=0.3, filename='VACF.png')
...
def plot_atomdos(atoms=range(24), filename='atom_dos.png'):
    ...
    series(xlabel='Frequency (THz)',
           ylabel='Phonon Density of States',
           datas=datas, linewidth=0.3, filename=filename, legend=False)
...
def plot_regiondos(regions, filename='region_dos.png'):
    ...
    series(xlabel='Frequency (THz)',
           ylabel='Phonon Density of States',
           datas=datas, linewidth=1, filename=filename, legend=True)
