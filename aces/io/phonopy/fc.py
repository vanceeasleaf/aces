...
# @Last Modified time: 2017-06-19 13:08:38
import numpy as np
    ...
    for i in range(natom):
        for j in range(natom):
            for k in range(natom):
                if np.allclose(
                        np.zeros([3, 3, 3]), fc3[i, j, k] / scale, atol=1e-04):
                    continue
                n += 1
                s += "\n%d\n" % n
                s += psm.maps2p(i, j, k)
...
class premitiveSuperMapper:
    ...
    def getS2p(self):
        ...
        vpos = pos * np.linalg.norm(cell, axis=1) / \
            np.linalg.norm(cellp, axis=1)
        v = np.floor(vpos)
...
def rotatefc2(t, direct=[0, 0, 1], file1='FORCE_CONSTANTS_2ND',
              file2='fc2new'):
    fc2 = readfc2(file1)
