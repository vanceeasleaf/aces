...
"""
    ...
    def testinit(self):
        ...
        self.assertEqual(
            "Spatial-averaged data for fix temp_profile and group main",
            fas.title)
    ...
    def testgetIStep(self):
        ...
        x = np.abs(fas.getIStep(1)[2][:, 1] -
                   np.array([0, 0, 0.00463008, 0.00275501])) < 0.01
        self.assertEqual(x.all(), True)
        x = np.abs(fas.getIStep(1)[2][:, 0] -
                   np.array([0, 0, 287.894, 291.343])) < 0.01
        self.assertEqual(x.all(), True)
    ...
    def testgetIbin(self):
        ...
        self.assertEqual(x.all(), True)
        x = np.abs(fas.getIbin(3)[:, 0] -
                   np.array([291.11, 291.343, 304.537])) < 0.01
        self.assertEqual(x.all(), True)
